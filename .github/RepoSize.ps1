...
foreach ($release in $releases) {
    $index++
    Write-Progress -Activity "Calculating asset sizes" -Status "Processing release $index of $releaseCount ($($release.tagName))" -PercentComplete (
        ($index / $releaseCount) * 100
    )
    ...
    Write-Output (
        "{0,-30} {1,15} {2,15}" -f $release.tagName,
        $releaseSizeMB,
        $releaseSizeGB
    )
}
...
Write-Output ""
Write-Output (
    "Total asset size across all releases: $totalSize bytes ($totalSizeMB MB, $totalSizeGB GB)"
)
