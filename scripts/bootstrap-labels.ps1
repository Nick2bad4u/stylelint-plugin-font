...
param(
    ...
    [string] $Repo, # If set, do not use --force (won't update labels that already exist).
    [Parameter(Mandatory = $false)]
    [switch] $NoForce, # If set, reads label names from the labeler config (top-level YAML keys).
    [Parameter(Mandatory = $false)]
    [switch] $FromLabelerYml, # Path to the labeler config (only used when -FromLabelerYml is set).
    [Parameter(Mandatory = $false)]
    [string] $LabelerYmlPath = ".github/labeler.yml", # If set with -FromLabelerYml, fails when labeler.yml contains labels not present in embedded defaults.
    [Parameter(Mandatory = $false)]
    [switch] $Strict, # If set, performs a read-only report (no gh writes), including remote label comparison (when possible).
    [Parameter(Mandatory = $false)]
    [switch] $AuditOnly, # Optional path to export the resolved label set as JSON.
    [Parameter(Mandatory = $false)]
    [string] $ExportJson, # Used for labels found in labeler.yml that don't exist in the embedded label set (when not -Strict).
    [Parameter(Mandatory = $false)]
    [ValidatePattern("^[0-9a-fA-F]{6}$")]
    [string] $FallbackColor = "cfd3d7", # Used for labels found in labeler.yml that don't exist in the embedded label set (when not -Strict).
    [Parameter(Mandatory = $false)]
...
$embeddedLabels = @(
    ...
    @{
        Name = "test"
        Color = "0e8a16"
        Description = "Needs tests or QA verification."
    },
    @{
        Name = "vulnerability"
        Color = "b60205"
    ...
    @{
        Name = "codex"
        Color = "1d76db"
        Description = "CodeX automation and tooling updates."
    },
    @{
        Name = "dependabot"
        Color = "06B1D0"
        Description = "Automated Dependabot updates."
    },
    @{
        Name = "dependencies"
        Color = "742D9A"
        Description = "Dependency maintenance work."
    },
    @{
        Name = "github-actions"
        Color = "5371BB"
        Description = "GitHub Actions workflows or runners."
    },
    @{
        Name = "npm"
        Color = "6F44BD"
        Description = "npm registry or packaging changes."
    },
    @{
        Name = "CI/CD"
        Color = "0b5394"
        Description = "Continuous integration and delivery updates."
    },
    @{
        Name = "configuration"
        Color = "fbca04"
        Description = "Configuration and tooling updates."
    }
)
...
if ($FromLabelerYml) {
    ...
    if ($unknownFromLabeler.Count -gt 0) {
        $msg = "labeler.yml contains labels not present in embedded defaults: " + (
            $unknownFromLabeler -join ", "
        )
        if ($Strict) {
            ...
            Write-Warn $msg
            Write-Warn (
                "Using fallback for unknown labels: Color={0}, Description='{1}'" -f $FallbackColor,
                $FallbackDescription
            )
        }
    }
...
if ($ExportJson) {
    ...
    $resolvedLabels
        ...
        | Set-Content -LiteralPath $ExportJson -Encoding UTF8
...
if ($AuditOnly) {
    Write-Warn "AuditOnly   : enabled (no changes will be made)"
}
...
if ($AuditOnly) {
    ...
    foreach ($l in $resolvedLabels) {
        [void] $desiredNames.Add([string] $l.Name)
    }

    if ($remote.Count -gt 0) {
        $remoteNames = [System.Collections.Generic.HashSet[string]]::new()
        foreach ($k in $remote.Keys) {
            [void] $remoteNames.Add([string] $k)
        }

        $missing = @()
        foreach ($n in $desiredNames) {
            if (-not $remoteNames.Contains($n)) {
                $missing += $n
            }
        }

        $extra = @()
        foreach ($n in $remoteNames) {
            if (-not $desiredNames.Contains($n)) {
                $extra += $n
            }
        }
        ...
        foreach ($l in $resolvedLabels) {
            ...
            $r = $remote[$name]
            if (
                ($r.Color -ne $l.Color) -or (
                    ($r.Description ?? "") -ne ($l.Description ?? "")
                )
            ) {
                $diffs++
                Write-Host (
                    "  - {0}: remote(color={1}, desc='{2}') != desired(color={3}, desc='{4}')" -f $name,
                    $r.Color,
                    ($r.Description ?? ""),
                    $l.Color,
                    ($l.Description ?? "")
